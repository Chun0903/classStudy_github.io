...
public class Ex05_Select {
	...
	private static void m7() {
		...
		System.out.println("[name]\t[jikwi]\t[city]\t[tel]\t\t[basicpay]");
		...
		try {
			...
			//rs == select 결과
			rs = stat.executeQuery(sql);	//레코드수?  > 확인불가
	...
	private static void m4() {
		...
		try {
			...
			//또다른 select 실행
			sql = "select * from tblInsa where buseo = '개발부' and jikwi <> '부장'";
	...
	private static void m1() {
		...
		try {
			...
			String sql = String.format("select count(*) as cnt from tblAdress");
