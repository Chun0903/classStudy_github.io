...
- 조건을 제시 > 모든 행을 대상으로 검사 > 만족하는 행은 결과셋 포함, 만족하지 못한 행을 결과셋 제외
- 비교 연산자 or 논리 연산자 사용

1.from 테이블
    - 가져올 테이블 선택(데이터 소스를 선택)
...
--tblInsa
--3. 부서가 '개발부' 이고 급여(basicpay)를 150만원 이상을 받는 지원을 가져오시오.
select * from tblinsa where buseo = '개발부' and basicpay >= 1500000;

--4. 급여(basicpwy) + 수당(sudang)을 200만원 이상 + '서울' 직원들 가져오시오.
select * from tblinsa where basicpay + sudang > 2000000 and city = '서울';

-- 조건절(where)에서 사용되는 여러 구문(연산자 or 함수 등등)
/*
between ~ and
- where 절에서 사용(조건으로 사용)
- 범위 조건
- 컬럼명 between A(최소값) and B(최댓값)
- A, B : 포함(inclusive)
- 가독성 향상
- 회사 : between 사용 금지
...
select * from tblCountry where population = null;       --X 못쓴다.
select * from tblCountry where population is null;      --인구수 미기재
select * from tblCountry where not population is null;  --X 잘 안씀
