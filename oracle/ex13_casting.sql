...
select to_char(sysdate, 'd') from dual;     -- 2    (이번주의 며칠)
...
select to_char(sysdate, 'ss') from dual;    -- 53   (초)
...
select 
    ...
    to_char(sysdate, 'hh24:mi:ss'),    --16:13:30
    to_char(sysdate, 'day am hh:mi:ss')--월요일 오후 04:14:19
from dual;
...
where to_char(ibsadate, 'd') = 2;             -- 월요일 입사
...
-- 형식문자열은 to_char()와 동일하게 사용

select sysdate, '2010-01-01' from dual;   --20/05/25, 10/01/01 이여야 날짜

select sysdate, to_date('2010-01-01', 'yyyy-mm-dd') from dual; --20/05/25, 10/01/01
...
select name, nvl(couple, '여자친구 없음'), nvl2(couple, '0','X') as 여자친구유무 from tblmen;
