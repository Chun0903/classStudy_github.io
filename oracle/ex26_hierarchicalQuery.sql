...
- 오라클 전용
- 레코드간의 관걔가 서로 상하 수직 구조의 관꺠가 있는 경우 적당한 구조의 결과셋을 만들어 준다.
- 부모와 자식 역할을 하는 레코드롤 구성된 테이블에서 사용한다.
- 카테고리, 답변형 게시판, 조직 주성원 등..
...
from tblComputer
    ...
    start with pseq is null
        connect by prior seq = pseq; -- on c1.pseq = ce.seq
