...
- 기존의 데이터베이스 구소를 개선하는 방법(좀 더 나은 구조로) 개선하는 방법
- 기존의 만든 테이블(비정형, 비정규화 상태) -> 정규화 -> 정규화 구조
- 제 1 정규화 -> 제 2 정규화 -> 제 3 정규화 등..
- 제 1~ 3 젇규화 + 역정규화

관계형 데이터베이스 시스템이 자향하는 데이터베이스의 상태
...
b. 갱신 이상(Update Anomaly)
    - 동일한 데이터가 2개 이상의 테이블에 존재 -> 그 중 1개를 수정 + 다른 1개를 수정 못함
        -> 두 데이터간의 이상 오류가 발생 -> 2개 데이터 중 어떤 데이터가 올바른 데이터인지 구분하지 못하는 이상 발생
...
- 정규화는 '부분 함수 종속이나 이행 함수 종속을 모두 없애고, 모든 컬럼의 관계를 완전 한수 종속'으로 만드는 작업
1. 완전 함수 종속, Full Funtional Dependency
2. 부분 함수 종속, Partial Funtional Dependency
3. 이행 함수 종속, Transitive Funtional Dependency


정규화
- 1NF ~3NF (Normal Form)
- 1개 테이블 -> (정규화) -> 2개 이상의 테이블
- 모든 정규화 단계는 기본 키가 아닌 컬럼들 기본키에 완전함수 종속을 만드는 작업
...
- 2NT는 복합키를 가지는 테이블에서만 발견된다.

제 3 정규화 2NF
- 기본 키가 아닌 다른 컬럼에게 종속되는 컬림이 존재하면 안된다.
