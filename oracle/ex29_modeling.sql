...
데이터베이스 구출
- 요구분석 + 데이터 수집 > 다량의 데이터 발생 > 저장하기 우한 데이터베이스의 조직화된 구조
    > 설계 > 구축
1. 데이터베이스 모델링
2. 데이터베이스 설계
3. 데이터베이스 구축
...
- 손, 오피스, 전문툴(exERD, SQL Dveloper, ER-WIN 등)

2. Entity, 엔티티
- 다른 Entity 와 분류(구분)될 수 있고,
...
4.b 후보 키, Candidate key     (= 기본키 + 대체키)
- 레코드와 레코드를 구분하는 역할
- 후보키들 중 대표로 선발된 키 -> 기본키
- 탈락된 키 -> 후보키

4.c 대체키, Alternate kry
- 후보키들 중 기본키를 제외한 나머지키

4.d 슈퍼키, Super key 
- 복합키, Composite key
- 1개 키만을 가지고 식별자 역할이 불가능한 경우 -> 2개 이상의 키를 조합해서 기본키로 만드는 경우
